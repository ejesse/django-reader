...
from feedreader.views import markRead, updateFeed, getFeed
...
class FeedparserTests(TestCase):
    ...
    def test_double_update(self):
        ...
        httpd.start()
        rssUrl = 'http://localhost:8097/src/feedreader/test_rss/test_1.rss'
    ...
    def test_changed_update(self):
        ...
        tempRss = Feed(name="Craig's List Free", url=rssUrl)
        tempRss.fetch()
        rssUrl = 'http://localhost:8097/src/feedreader/test_rss/test_1_update.rss'
    ...
    def test_read_functions(self):
        ...
        httpd.start()
        rssUrl = 'http://localhost:8097/src/feedreader/test_rss/test_2.rss'
...
# View testing
    ...
    def test_view_ajax_mark_read(self):
        ...
        tempRss = Feed(name="Temp", url=rssUrl)
        tempRss.fetch()
        response = self.client.get(reverse(markRead, args=(1,)),
                                   HTTP_X_REQUESTED_WITH='XMLHttpRequest')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(tempRss.unread(), 0)  
        # bad request
        response = self.client.get(reverse(markRead, args=(10,)),
                                   HTTP_X_REQUESTED_WITH='XMLHttpRequest')
    ...
    def test_view_ajax_getFeed(self):
        ...
        tempRss.fetch()
        tempRss.save()
        response = self.client.get(reverse(getFeed, args=(1,)),
                                   HTTP_X_REQUESTED_WITH='XMLHttpRequest')
#        print response.content
    ...
    def test_view_ajax_updateFeed(self):
        ...
        tempRss = Feed(name="Temp", url=rssUrl)
        tempRss.save()
        response = self.client.get(reverse(updateFeed, args=(1,)),
                                   HTTP_X_REQUESTED_WITH='XMLHttpRequest')
        self.assertEqual(len(response.context['FeedList']), 1)
#        print response.content